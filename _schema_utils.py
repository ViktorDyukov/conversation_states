...
class SchemaCoercionMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self._field_coercers: Optional[dict[str,
    ...
    def _coerce(self, input_data: Any, depth: Optional[int] = None) -> Any:
        print("🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬🛬")
        print("🛬 Input data for coercion:", input_data)
        ...
        for k, v in input_data.items():
            ...
            print(f"🚨 Trying to coerce {v} into {self.schema}")
        ...
        if origin is Union:
            ...
            for ix, arg in enumerate(uargs):
                if arg is type(None):
                    ...
                    subs.append(
                        self._build_coercer(
                            arg, depth - 1, throw=ix < len(uargs) - 1)
                    )

            def union_coercer(v: Any, d: Any) -> Any:
                print(f"\n🔥🔥🔥 UNION COERCER TRYING VALUE:\n{v}\n")
                if v is None and none_in_union:
                    return None
                err = None
                sub_type_pairs = list(
                    zip(subs, [arg for arg in uargs if arg is not type(None)]))
                for i, sp in enumerate(subs):
                    type_hint = sub_type_pairs[i][1]
                    print(
                        f"\n⚙️ TRYING TO COERCE INTO: {type_hint.__name__ if hasattr(type_hint, '__name__') else type_hint}\n")
                    try:
                        result = sp(v, d - 1)
                        print(f"\n✅ SUCCESSFULLY COERCED INTO: {arg}\n")
                        return result
